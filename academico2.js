//copia para testes de implementação academico.js
...
//
function carregarDados(string) { //ler dados...
...
function prepararDadosSalvar(alunos, materias) { //salva...
...
function prepararDadosSalvarAluno(alunos) { //salva...
...
function prepararDadosSalvarMateria(materias) { //salva...
...
for(;escolhaUsuario !== '9';){
    ...
    switch (escolhaUsuario) { //swtich (escolha da opção no menu)
        case '1':
            var entradaOp = require('readline-sync');
            var op = entradaOp.question('');
            console.log(   '\n==================================================================\n \t Menu Alunos\n======================== Digite uma Opção ========================\n\t1 ► Listar Alunos\n\t2 ► Cadastrar\n\t3 ► Retornar ao menu');
                //switch dentro de outro... kkkk ^.-.^ xD
                switch(op){
                    case '1':
                        console.log('\t Listar Alunos Opção 1 selecionada...\t\n >');
                        ListaAlunos();
                        break;
                    case '2':
                        AdicionaAluno();
            ...
            //mensagem = menuAlunos();//função menu alunos implementar...
            break;
        case '2':
            //mensagem = Materias(); // implementar...
            console.log('\t Menu Materias... \n\t 1 > Listar Matéria\n\t 2 > Cadastrar Matéria\n\t 3 > Voltar ao Menu');//implementar...
            var entradaOp2 = require('readline-sync'); //entrada para menu interno...
            var op = entradaOp2.question('');//ler menu interno...
            switch(op){
                case '1':
                    console.log('\t Listar Matérias...\t\n >');
                    ListaMaterias();//alterado.
                    break;
                case '2':
                    AdicionarMatéria();
                break;
                case '3':
                    ...
                    menuInicial();//volta para o menu

            }
            break;
        case '3':
            mensagem = LancaNotas(); //implementar...
            break;
        case '4':
            colegio.imprimirBoletim(alunos);//imprime o boletão...
            mensagem = ''; //retorna a mensagem ...implementar...
            break;
        case '9': //encerra e sair...
            console.log('Saindo...');
            break;
        default:
...
function menuInicial() {//essa bagaça vai ficar repetitiva, mas f0d4-s3...
    console.log('==================================================================');
...
function AdicionarMatéria(){
    var quantidadeparadigitar = require('readline-sync');
    terminal.imprimir('\n > Informe a quantidde de Matérias a cadastrar: \n');
    var qtdMaterias = quantidadeparadigitar.question('');//recebe a quantidade desejada...
    const materia = []; //array materias ... alterado
    const entrada = terminal.escolherTerminal();//função que vem de terminal captura entrada
                        
        for(let j = 0; j < qtdMaterias; j ++){
            terminal.imprimir('\n================================================================');
            terminal.imprimir(' Por favor informe o nome da Matéria: ►');
            terminal.imprimir('=================================================================');
            let nome = terminal.lerALinhaInteira(entrada); //alterado...
            /* 
            essa opçao não esta totalmente implementada ou funcional...               
               */             
            materia.push(materia); //adiciona materia...
            terminal.imprimir('\n-----------------------------------------------------------------');
            //fins de teste... (backup requerido antes e ao testar...)
            const dadosPreparados = prepararDadosSalvarMateria(materia);
            fs.writeFileSync('./../arquivo/teste.denylson', dadosPreparados, 'utf-8');//sobrescreve o arquivo... não utilizar...
